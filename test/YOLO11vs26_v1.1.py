...
for target_idx in range(TARGET_START, TARGET_END):
    try:
        ...
        # 파일명 (Tracking이 빠졌으므로 Detection으로 명시하면 좋습니다)
        video_filename = f"Comparison_v1.1.mp4"
        output_video_path = str(OUTPUT_DIR / video_filename)
        
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_video_path, fourcc, 30.0, (w * 2, h * 2))
        
        print(f"\n▶️ [Target {target_idx}] Pure Detection 분석 시작: {len(frame_files)} Frames")
        ...
        for frame_path in tqdm(frame_files, desc=f"Target {target_idx}", unit="frame"):
            ...
            for i, model in enumerate(models):
                ...
                # [수정됨] track() -> predict()로 변경 (ID 추적 기능 제거)
                # persist=True 옵션도 제거했습니다.
                result = model.predict(input_img, imgsz=640, classes=[0], device=device, 
                                       verbose=False)[0]
                ...
                # 시각화 (ID 없이 Box와 Skeleton만 그려집니다)
